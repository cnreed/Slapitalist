...
public class GameTest {
    ...
    public static Company Rahoi, Imperial, Worldwide, Tower, American,
    ...
    public static int[][] playerStockList; // numplayers then companyCID

    public GameTest(int x, int y, Player[] players, Board board) {
    ...
    public void payPlayers() {
        ...
        for (int i = 0; i < players.length; i++) {
            payPlayersHelper(players[i], i);
        }

    }

    public void payPlayersHelper(Player player, int index) {

        for (int i = 0; i < playerStockList[index].length; i++) {
            int stock = playerStockList[index][i];

            Company company = companyList.get(i);
            int sharePrice = company.getSharePrice();

            player.updateCash(stock * sharePrice);
            company.addStockBack(stock);
            // sellBackStock(company, index);
            System.out.println("player cash: " + player.getCash());

        }
        // sellBackStock(null, 0);
    ...
    public ArrayList<Player> determineWinner() {
        ...
        if (equalSize.size() > 0) {
            for (int i = 0; i < equalSize.size(); i++) {
                // TODO Special case for this one.
                System.out.print(equalSize.get(i).getName() + ", ");
            }
            return equalSize;
        } else {
    ...
    private void playerTurnPartTwo(Player player, Board board) {
        ...
        if (buyYesOrNo == 1) {
            ...
            while (stockTotal < 3) {
                ...
                int companyStockPrice = companyToBuy.getSharePrice();
...
//                selectionCount = 0;
    ...
    public void merge(ArrayList<Company> companies, Tile mTile) {
        ...
        log.debug("Starting the mergeTurn: ");
    ...
    public static int[][] getPlayerStockList() {
