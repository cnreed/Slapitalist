...
import Game.GameInterface.ICompany;
...
public class Company implements ICompany {
    ...
    private ArrayList<Tile> companyTiles; // all tiles owned by the
    // company
//    private static String companyColor; // specific color associated with the
    ...
    private boolean isSafe; // start false
    private boolean gameEndable; // start false
    private boolean onBoard; // start false
...
//        Company.companyColor = companyColor;
    ...
    public void incrementSize() {
        companySize++;
        if (companySize == 11) {
            log.debug("Company: " + companyName + " is now safe");
            isSafe = true;
            // TODO - Set unplayable
    ...
    public void dissolve() { // drops all backend values to 0, which cascades to
        // prices, etc.
    ...
    public Tile getTile(int loc) {
        Tile tile = companyTiles.get(loc);
        return tile;
    }


    public boolean containsTile(Tile tile) {
        return companyTiles.contains(tile);
    }
